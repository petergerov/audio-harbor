...
struct AnalogVUPair: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                EngravedLabel(text: "Meters", size: 9)
                Spacer()
                EngravedLabel(text: "VU  ·  0 = −18 dBFS", size: 8)
            }
            HStack(spacing: compact ? 10 : 14) {
                AnalogVUMeter(label: "L", level: left, compact: compact)
                AnalogVUMeter(label: "R", level: right, compact: compact)
            }
        }
    }
}
